...
function deleteConfirm(name,path){
	var flag=window.confirm("您确定要删除联系人：“"+name+"”？");
	if(true==flag){
		$.post(path);
							...
							<TBODY>
								<TR>
									...
									<FORM id="pageForm" name="customerForm"	
											action="<c:url value='/LinkManAction_list'/>" method=post>
											<%-- 保存currentPage --%>
											...
											<TBODY>
												<TR>
													...
													<TD>
														...
														style="WIDTH: 80px" maxLength=50 name="lkm_name" value="${requestScope.lkm_name }"/>
													</TD>
													<td>客户名称：</td>
													<td>
														<input type="hidden" name="customer.cust_id" id="cust_id" value="${customer.cust_id }">
														<INPUT class=textbox readonly="readonly" name="cust_name" id="cust_name" style="WIDTH: 80px" maxLength=50 value="${param['cust_name'] }"/>
													</td>
													<td>
										...
										<TABLE id=grid
											...
											<TBODY>
												...
												<TR
													...
													&nbsp;&nbsp;
													<a href="javaScript:void(0)" onclick='deleteConfirm("<s:property value="#linkMan.lkm_name"/>","${pageContext.request.contextPath }/LinkManAction_delete?lkm_id=<s:property value="#linkMan.lkm_id"/>")'>删除</a>
													</TD>
												</TR>
